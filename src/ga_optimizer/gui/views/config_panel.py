# config_panel.py
# Panel konfiguracji z przewijanym formularzem górnym i notebookiem metod.
...
class ScrollableSection(ttk.Frame):
    def __init__(self, parent, padding=10, scrollbar_margin=24, right_inner_gap=22):
        # Uniwersalna przewijalna sekcja oparta o Canvas + Frame.
        super().__init__(parent)
        self.scrollbar_margin = scrollbar_margin
        self.right_inner_gap = right_inner_gap
    ...
    def _on_frame_configure(self, _event=None):
        # Dopasowuje scrollregion do aktualnej zawartości i aktualizuje scrollbar.
        self.canvas.configure(scrollregion=self.canvas.bbox("all"))
        self.after_idle(self.update_scrollbar_visibility)

    def _on_canvas_configure(self, event):
        # Rozciąga wewnętrzną ramkę do szerokości canvasa,
        # ale zostawia trochę luzu przed scrollbarem.
        inner_width = max(100, event.width - self.right_inner_gap)
        self.canvas.itemconfigure(self._window_id, width=inner_width)
        self.after_idle(self.update_scrollbar_visibility)

    def update_scrollbar_visibility(self) -> None:
        # Pokazuje scrollbar tylko wtedy, gdy zawartość realnie nie mieści się z zapasem.
        try:
        ...
        should_show = inner_height > max(1, canvas_height - self.scrollbar_margin)
    ...
    def refresh_mousewheel_bindings(self) -> None:
        # Odświeża bindowanie scrolla dla całej sekcji i wszystkich dzieci.
        self._bind_mousewheel_recursive(self)
        self._bind_mousewheel_recursive(self.inner)
        self.after_idle(self.update_scrollbar_visibility)

    def _block_widget_mousewheel_change(self, widget) -> None:
        # Blokuje zmianę wartości widgetu po scrollu myszy.
    ...
    def _bind_mousewheel_recursive(self, widget) -> None:
        ...
        if isinstance(widget, ttk.Combobox):
            self._block_widget_mousewheel_change(widget)

        for child in widget.winfo_children():
            self._bind_mousewheel_recursive(child)

    def _activate_mousewheel(self, _event=None):
        # Aktywuje przewijanie po wejściu kursora nad sekcję.
        if not self._scrollbar_visible or self._mousewheel_active:
            return
    ...
    def _deactivate_mousewheel(self, _event=None):
        # Wyłącza globalne bindowanie scrolla po opuszczeniu sekcji.
...
class ConfigPanel:
    ...
    def build(self) -> None:
        ...
        self.frame.rowconfigure(0, weight=7, uniform="config_rows")
        self.frame.rowconfigure(1, weight=3, uniform="config_rows")

        self.top_frame = ttk.LabelFrame(self.frame, text="Konfiguracja", padding=8)
        self.top_frame.grid(row=0, column=0, sticky="nsew")
        self.top_frame.columnconfigure(0, weight=1)
        self.top_frame.rowconfigure(0, weight=1)

        self.top_section = ScrollableSection(self.top_frame, padding=4)
        self.top_section.grid(row=0, column=0, sticky="nsew")

        top = self.top_section.inner
        ...
        row = self._add_entry(
            top,
            row,
            GENERAL_FIELD_SPECS["n_vars"]["label"],
            self.vm.n_vars,
            "n_vars",
            store_as="n_vars",
        )
        ...
        row = self._add_entry(
            top,
            row,
            GA_MAIN_FIELD_SPECS["population"]["label"],
            self.vm.population,
            "population",
        )
        ...
        row = self._add_entry(
            top,
            row,
            GA_MAIN_FIELD_SPECS["epochs"]["label"],
            self.vm.epochs,
            "epochs",
        )
        row = self._add_entry(
            top,
            row,
            GA_MAIN_FIELD_SPECS["run_count"]["label"],
            self.vm.run_count,
            "run_count",
        )
        row = self._add_entry(
            top,
            row,
            GA_MAIN_FIELD_SPECS["seed"]["label"],
            self.vm.seed,
            "seed",
        )
        row = self._add_separator(top, row)
        ...
        self.selection_tab = ScrollableSection(self.nb, padding=10)
        self.crossover_tab = ScrollableSection(self.nb, padding=10)
        self.mutation_tab = ScrollableSection(self.nb, padding=10)
    ...
    def refresh_layout(self) -> None:
        # Odświeża przewijanie i widoczność scrollbarów po resize.
        if self.top_section is not None:
            self.top_section.refresh_mousewheel_bindings()

        for tab in (self.selection_tab, self.crossover_tab, self.mutation_tab):
            if tab is not None:
                tab.refresh_mousewheel_bindings()
    ...
    def _add_combo(self, parent, row, label, var, values):
        ...
        cb.grid(row=row, column=1, sticky="w", pady=4, padx=(0, 10))
        return row + 1
    ...
    def _add_range(self, parent, row):
        ttk.Label(parent, text="Początek przedziału:").grid(
            row=row,
            column=0,
            sticky="w",
            pady=4,
        ...
        start_entry = ttk.Entry(parent, textvariable=self.vm.range_start, width=14)
        start_entry.grid(row=row, column=1, sticky="w", pady=4, padx=(0, 10))
        self._register_entry_widget("range_start", start_entry)

        row += 1

        ttk.Label(parent, text="Koniec przedziału:").grid(
            row=row,
            column=0,
            sticky="w",
            pady=4,
        ...
        end_entry = ttk.Entry(parent, textvariable=self.vm.range_end, width=14)
        end_entry.grid(row=row, column=1, sticky="w", pady=4, padx=(0, 10))
    ...
    def _add_precision_block(self, parent, row):
        ...
        box.grid(row=row, column=1, sticky="w", pady=4, padx=(0, 10))
        ...
        self.precision_numeric_entry = ttk.Entry(parent, textvariable=self.vm.precision_numeric, width=26)
        self.precision_numeric_entry.grid(row=row, column=1, sticky="w", pady=4, padx=(0, 10))
        ...
        self.precision_bits_entry = ttk.Entry(parent, textvariable=self.vm.precision_bits, width=26)
        self.precision_bits_entry.grid(row=row, column=1, sticky="w", pady=4, padx=(0, 10))
        ...
        if not specs:
            ...
            for spec in specs:
                ...
                if spec.get("type") == "enum":
                    ...
                    entry = ttk.Entry(frame, textvariable=self.vm.method_params[spec["key"]], width=26)
                    entry.grid(row=row, column=1, sticky="w", pady=4, padx=(0, 10))
