...
def dispatch_crossover(
    ...
    crossovered_chromosomes = [chromosome.copy() for chromosome in chromosomes]

    match method_name:
        case "one_point":
            crossovered_chromosomes = one_point_crossover(chromosomes, config_dict)

        case "two_point":
            crossovered_chromosomes = two_point_crossover(chromosomes, config_dict)

        case "three_point":
            crossovered_chromosomes = three_point_crossover(chromosomes, config_dict)

        case "multi_point":
            crossovered_chromosomes = multi_point_crossover(chromosomes, config_dict)

        case "uniform":
            crossovered_chromosomes = uniform_crossover(chromosomes, config_dict)

        case "shuffle":
            crossovered_chromosomes = shuffle_crossover(chromosomes, config_dict)

        case "granular":
            crossovered_chromosomes = granular_crossover(chromosomes, config_dict)

        case "reduced_surro":
            crossovered_chromosomes = reduced_surro_crossover(chromosomes, config_dict)

        case "disruptive":
            crossovered_chromosomes = disruptive_crossover(chromosomes, config_dict)

        case "majority":
            crossovered_chromosomes = majority_crossover(chromosomes, config_dict)

        case "arithmetic":
            crossovered_chromosomes = arithmetic_crossover(chromosomes, config_dict)

        case "segmented":
            crossovered_chromosomes = segmented_crossover(chromosomes, config_dict)

        case _:
            debug_print(verbose, f"[Ostrzeżenie] Nieznana metoda krzyżowania: {method_name}. Zostawiam chromosomy bez zmian.")

    debug_print(verbose, f"[Crossover] Chromosomy dla metody: {method_name}")
